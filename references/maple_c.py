...
class MapleExplainer:
    ...
    def attributions(self, X, multiply_by_input=False):
        ...
        # out = [np.zeros(X.shape) for j in range(self.num_class)]
        out = []
        for i in tqdm(range(X.shape[0])):
            ...
            if multiply_by_input:
                ...
                out[i] = np.einsum('DC,D->DC', out[i], (X[i] - self.data_mean))  # out[i] * (X[i] - self.data_mean)

        return out[0] if self.flat_out else np.array(out)
...
class MAPLE:
    def __init__(
        ...
        for retain in range(1, num_features + 1):
            ...
            lr_predictions = np.empty([num_val, num_cla], dtype=float)
    ...
    def explain(self, x):
        ...
        coefs = np.zeros([self.num_features + 1, self.num_cla])
        coefs[0] = lr_model.intercept_
        coefs[np.sort(mostImpFeats[0 : self.retain]) + 1] = lr_model.coef_.T
...
class Maple_c:
    ...
    def explain(self, x):
        shap_values = self.explainer.attributions(x, multiply_by_input=True)    #.values
